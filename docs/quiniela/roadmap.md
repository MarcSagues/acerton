...
| Sistema de referidos (link único, registro invitado↔invitador) | ⬜ | Sin empezar. Riesgo principal: abuso multicuenta — mitigado por la caída de XP descrita arriba, sin necesidad de verificación de dispositivo/IP. **Requisitos añadidos 2026-09-16 (a petición explícita del usuario, todavía sin implementar — solo planteamiento)**: (1) el código de referido debe ser el mismo valor tanto si se comparte como link (`?ref=CODIGO` o similar) como si se introduce a mano — un único identificador por usuario, no dos sistemas paralelos; (2) además de entrar por link, tiene que poder entrarse tecleando el código directamente; el campo para teclearlo debe vivir siempre en Ajustes (o un sitio de baja visibilidad), no en un flujo destacado; (3) pensado para en el futuro poder aplicar descuentos de Premium (Sprint 11) según de qué usuario venga el referido — el modelo de datos de esta tarea debería guardar la relación referido↔referidor de forma que Premium pueda consultarla más adelante sin rehacer el sistema, aunque la lógica de descuento en sí es tarea del Sprint 11, no de este sprint. Sin decidir todavía: formato exacto del código, límites de uso, y el mecanismo de descuento en sí (eso es alcance de Sprint 11). Ver `decisions.md` para el registro de este requisito. |
