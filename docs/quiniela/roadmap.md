...
| Orden estable de jornadas por cierre de pronósticos | ✅ backend | Resuelto con el usuario 2026-09-19 (ver `decisions.md`): la jornada "actual" a mostrar/puntuar (`getCurrentMatchdayForCompetition`) pasa a elegirse por cierre más próximo (`closesAt`), no por número de orden — antes era deliberadamente lo contrario. `canAcceptPredictions` no necesitó cambios: su regla ya cubría el ejemplo dado (jornada aplazada sigue abierta, jornadas muy por delante siguen bloqueadas). Sin verificar en navegador (cambio de backend puro, sin datos de prueba con un aplazamiento real). |
