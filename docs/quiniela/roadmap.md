...
| Catálogo ampliado (12 insignias propuestas) | 🟡 | Añadidas 6 de las 7 que faltaban (2026-09-15, a petición del usuario: "5 me parecen pocas"), con checker y progreso propios en `BadgesService`: `STREAK_25` (Incombustible, 25 jornadas seguidas), `PREDICTIONS_100` (Centenario, 100 pronósticos en un grupo), `ONE_X_TWO_HITS_25`/`ONE_X_TWO_HITS_100` (Buen ojo/Experto en 1X2, solo en grupos modo 1X2), `EXACT_SCORE_HIT_1`/`EXACT_SCORE_HITS_10` (Al milímetro/Francotirador, marcador exacto acertado — no solo el ganador —, solo en grupos resultado exacto). Quedan 11 insignias en total. Falta solo "Campeón" (primer premio de temporada): depende de temporada cerrada de verdad + modelo de trofeos (Sprint 6, 🔒 sin modelo todavía) — no se ha inventado una condición sustituta. Ninguna de las 6 nuevas tiene arte 3D propio (`assets/art.js`/`insignias/3d/*.png`, ver `HANDOFF §18` — los 5 motivos originales se dieron por fijos): usan el icono genérico de fallback ya existente (medalla conseguida / candado pendiente, ver `badge-art.ts` → `null` y `profile-badges.component.html`). 10 tests unitarios nuevos + verificado en navegador contra datos reales del grupo de desarrollo (progreso real: 5/25 Buen ojo, 21/100 Centenario, 3/25 Incombustible, 0/10 Francotirador). Sin verificar iOS/Android. |
