...
| Puntito rojo de nivel sin reclamar (mismo patrón que Jornada) | ✅ web | Mismo estilo `.pending-dot` que ya usan Jornada/Grupos en la barra inferior, reutilizado en 3 sitios más: pestaña Perfil de la barra inferior, el nodo concreto del recorrido de nivel, y el icono de la tarjeta de detalle — los tres leen `NotificationsFeedService.unreadLevelUps()`. La campanita de arriba usa su contador de avisos sin leer ya existente (sin campo nuevo). |
