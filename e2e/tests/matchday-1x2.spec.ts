...
import { createGroup, login, uniqueGroupName } from './helpers';

test.describe('Jornada — modo 1X2', () => {
  test('elegir un pronóstico 1X2 lo guarda y persiste tras recargar', async ({ page }) => {
    await login(page);
    await createGroup(page, { name: uniqueGroupName('Jornada 1X2'), scoringMode: 'ONE_X_TWO' });
  ...
  test('el contador de la jornada sube según se completan pronósticos', async ({ page }) => {
    await login(page);
    await createGroup(page, { name: uniqueGroupName('Contador 1X2'), scoringMode: 'ONE_X_TWO' });
    await page.goto('/matchday');
